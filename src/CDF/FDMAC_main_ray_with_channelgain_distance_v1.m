...
pro_down=0.8;
Monte_Carlo_T=1000;
delta=100;% numbers of  CDF interval
distance_delta=sqrt(2)*side_of_square/100;% CDF interval (meter)
...
CDF_IA_DU_AP=zeros(2,delta,K); %3rd-dimansion: 1:x-axis of CDF ;2: y-axis of CDF
CDF_IM_DU_AP=zeros(2,delta,K);
CDF_SM_DU_AP=zeros(2,delta,K);
CDF_SMM_DU_AP=zeros(2,delta,K);
CDF_SRM_DU_AP=zeros(2,delta,K);

CDF_IA_DU_STA=zeros(2,delta,K);
CDF_IM_DU_STA=zeros(2,delta,K);
CDF_SM_DU_STA=zeros(2,delta,K);
CDF_SMM_DU_STA=zeros(2,delta,K);
CDF_SRM_DU_STA=zeros(2,delta,K);

CDF_IA_UD_AP=zeros(2,delta,K);
CDF_IM_UD_AP=zeros(2,delta,K);
CDF_SM_UD_AP=zeros(2,delta,K);
CDF_SMM_UD_AP=zeros(2,delta,K);
CDF_SRM_UD_AP=zeros(2,delta,K);

CDF_IA_UD_STA=zeros(2,delta,K);
CDF_IM_UD_STA=zeros(2,delta,K);
CDF_SM_UD_STA=zeros(2,delta,K);
CDF_SMM_UD_STA=zeros(2,delta,K);
CDF_SRM_UD_STA=zeros(2,delta,K);
...
for k=1:K
    ...
    for T=1:Monte_Carlo_T
        %% andom deploy STAs in a square area
        coordinate=unifrnd(-side_of_square/2,side_of_square/2,[2,number_STAs]);
        %% calculate distances
    ...
    [y_CDF_IA_DU_AP x_CDF_IA_DU_AP]=ecdf(record_distance_IA_DU(1,:));
    [y_CDF_IM_DU_AP x_CDF_IM_DU_AP]=ecdf(record_distance_IM_DU(1,:));
    [y_CDF_SM_DU_AP x_CDF_SM_DU_AP]=ecdf(record_distance_SM_DU(1,:));
    [y_CDF_SMM_DU_AP x_CDF_SMM_DU_AP]=ecdf(record_distance_SMM_DU(1,:));
    [y_CDF_SRM_DU_AP x_CDF_SRM_DU_AP]=ecdf(record_distance_SRM_DU(1,:));
    
    [y_CDF_IA_DU_STA x_CDF_IA_DU_STA]=ecdf(record_distance_IA_DU(2,:));
    [y_CDF_IM_DU_STA x_CDF_IM_DU_STA]=ecdf(record_distance_IM_DU(2,:));
    [y_CDF_SM_DU_STA x_CDF_SM_DU_STA]=ecdf(record_distance_SM_DU(2,:));
    [y_CDF_SMM_DU_STA x_CDF_SMM_DU_STA]=ecdf(record_distance_SMM_DU(2,:));
    [y_CDF_SRM_DU_STA x_CDF_SRM_DU_STA]=ecdf(record_distance_SRM_DU(2,:));
    
    [y_CDF_IA_UD_AP x_CDF_IA_UD_AP]=ecdf(record_distance_IA_UD(1,:));
    [y_CDF_IM_UD_AP x_CDF_IM_UD_AP]=ecdf(record_distance_IM_UD(1,:));
    [y_CDF_SM_UD_AP x_CDF_SM_UD_AP]=ecdf(record_distance_SM_UD(1,:));
    [y_CDF_SMM_UD_AP x_CDF_SMM_UD_AP]=ecdf(record_distance_SMM_UD(1,:));
    [y_CDF_SRM_UD_AP x_CDF_SRM_UD_AP]=ecdf(record_distance_SRM_UD(1,:));
    
    [y_CDF_IA_UD_STA x_CDF_IA_UD_STA]=ecdf(record_distance_IA_UD(2,:));
    [y_CDF_IM_UD_STA x_CDF_IM_UD_STA]=ecdf(record_distance_IM_UD(2,:));
    [y_CDF_SM_UD_STA x_CDF_SM_UD_STA]=ecdf(record_distance_SM_UD(2,:));
    [y_CDF_SMM_UD_STA x_CDF_SMM_UD_STA]=ecdf(record_distance_SMM_UD(2,:));
    [y_CDF_SRM_UD_STA x_CDF_SRM_UD_STA]=ecdf(record_distance_SRM_UD(2,:));
    
    CDF_IA_DU_AP(:,1:size(x_CDF_IA_DU_AP),k)=[x_CDF_IA_DU_AP.' ;y_CDF_IA_DU_AP.'];
    CDF_IM_DU_AP(:,1:size(x_CDF_IM_DU_AP),k)=[x_CDF_IM_DU_AP.' ;y_CDF_IM_DU_AP.'];
    CDF_SM_DU_AP(:,1:size(x_CDF_SM_DU_AP),k)=[x_CDF_SM_DU_AP.' ;y_CDF_SM_DU_AP.'];
    CDF_SMM_DU_AP(:,1:size(x_CDF_SMM_DU_AP),k)=[x_CDF_SMM_DU_AP.' ;y_CDF_SMM_DU_AP.'];
    CDF_SRM_DU_AP(:,1:size(x_CDF_SRM_DU_AP),k)=[x_CDF_SRM_DU_AP.' ;y_CDF_SRM_DU_AP.'];
    
    CDF_IA_DU_STA(:,1:size(x_CDF_IA_DU_STA),k)=[x_CDF_IA_DU_STA.' ;y_CDF_IA_DU_STA.'];
    CDF_IM_DU_STA(:,1:size(x_CDF_IM_DU_STA),k)=[x_CDF_IM_DU_STA.' ;y_CDF_IM_DU_STA.'];
    CDF_SM_DU_STA(:,1:size(x_CDF_SM_DU_STA),k)=[x_CDF_SM_DU_STA.' ;y_CDF_SM_DU_STA.'];
    CDF_SMM_DU_STA(:,1:size(x_CDF_SMM_DU_STA),k)=[x_CDF_SMM_DU_STA.' ;y_CDF_SMM_DU_STA.'];
    CDF_SRM_DU_STA(:,1:size(x_CDF_SRM_DU_STA),k)=[x_CDF_SRM_DU_STA.' ;y_CDF_SRM_DU_STA.'];
    
    
    
    CDF_IA_UD_AP(:,1:size(x_CDF_IA_UD_AP),k)=[x_CDF_IA_UD_AP.' ;y_CDF_IA_UD_AP.'];
    CDF_IM_UD_AP(:,1:size(x_CDF_IM_UD_AP),k)=[x_CDF_IM_UD_AP.' ;y_CDF_IM_UD_AP.'];
    CDF_SM_UD_AP(:,1:size(x_CDF_SM_UD_AP),k)=[x_CDF_SM_UD_AP.' ;y_CDF_SM_UD_AP.'];
    CDF_SMM_UD_AP(:,1:size(x_CDF_SMM_UD_AP),k)=[x_CDF_SMM_UD_AP.' ;y_CDF_SMM_UD_AP.'];
    CDF_SRM_UD_AP(:,1:size(x_CDF_SRM_UD_AP),k)=[x_CDF_SRM_UD_AP.' ;y_CDF_SRM_UD_AP.'];
    
    CDF_IA_UD_STA(:,1:size(x_CDF_IA_UD_STA),k)=[x_CDF_IA_UD_STA.' ;y_CDF_IA_UD_STA.'];
    CDF_IM_UD_STA(:,1:size(x_CDF_IM_UD_STA),k)=[x_CDF_IM_UD_STA.' ;y_CDF_IM_UD_STA.'];
    CDF_SM_UD_STA(:,1:size(x_CDF_SM_UD_STA),k)=[x_CDF_SM_UD_STA.' ;y_CDF_SM_UD_STA.'];
    CDF_SMM_UD_STA(:,1:size(x_CDF_SMM_UD_STA),k)=[x_CDF_SMM_UD_STA.' ;y_CDF_SMM_UD_STA.'];
    CDF_SRM_UD_STA(:,1:size(x_CDF_SRM_UD_STA),k)=[x_CDF_SRM_UD_STA.' ;y_CDF_SRM_UD_STA.'];
    
    fprintf('The k=%i /%i \n',k,K);
end

save distance_v1
save distance_v1_data CDF_IA_DU_AP CDF_IM_DU_AP CDF_SM_DU_AP CDF_SMM_DU_AP CDF_SRM_DU_AP...
 CDF_IA_DU_STA CDF_IM_DU_STA CDF_SM_DU_STA CDF_SMM_DU_STA CDF_SRM_DU_STA...
 CDF_IA_UD_AP CDF_IM_UD_AP CDF_SM_UD_AP CDF_SMM_UD_AP CDF_SRM_UD_AP...
 CDF_IA_UD_STA CDF_IM_UD_STA CDF_SM_UD_STA CDF_SMM_UD_STA CDF_SRM_UD_STA